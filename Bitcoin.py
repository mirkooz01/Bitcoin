...
def fetch_bitcoin_price():
    """Ottiene il prezzo di Bitcoin in EUR."""
    try:
        response = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=eur")
        data = response.json()
        return f"{data['bitcoin']['eur']:.2f}"
    except Exception:
        return "Errore"

def draw_character(canvas, x, y, char, color="red"):
...
def update_display():
    """Aggiorna il display con il valore corrente di Bitcoin."""
    global canvas
...
# Canvas per i LED
canvas = tk.Canvas(root, width=600, height=100, bg="#1e1e1e", highlightthickness=0)
canvas.pack()
